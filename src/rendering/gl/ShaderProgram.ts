...
class ShaderProgram {
  ...
  constructor(shaders: Array<Shader>) {
    ...
    this.attrOrient = gl.getAttribLocation(this.prog, "vs_Orient");
  ...
  draw(d: Drawable) {
    ...
    if (this.attrOrient != -1 && d.bindOrient()) {
      gl.enableVertexAttribArray(this.attrOrient);
      gl.vertexAttribPointer(this.attrOrient, 3, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(this.attrOrient, 1); // Advance 1 index in orient VBO for each drawn instance
    }
    ...
    if (this.attrOrient != -1) gl.disableVertexAttribArray(this.attrOrient);
